...
document.getElementById('locationForm').addEventListener('submit', async (e) => {
    ...
    try {
        if (locationId) {
            ...
            await apiCall('http://10.160.65.43:3000/api/locations', 'POST', locationData);
...
async function loadUsers() {
    try {
        const data = await apiCall('http://10.160.65.43:3000/api/users');
...
document.getElementById('userForm').addEventListener('submit', async (e) => {
    ...
    try {
        if (userId) {
            ...
            await apiCall('http://10.160.65.43:3000/api/users', 'POST', userData);
...
async function populateFilters() {
    try {
        const [usersData, locationsData] = await Promise.all([
            apiCall('http://10.160.65.43:3000/api/users'),
            apiCall('http://10.160.65.43:3000/api/locations')
        ]);
...
async function loadAttendanceForEdit(attendanceId) {
    try {
        const data = await apiCall('http://10.160.65.43:3000/api/attendance');
...
document.getElementById('attendanceForm').addEventListener('submit', async (e) => {
    ...
    try {
        if (attendanceId) {
            ...
            await apiCall('http://10.160.65.43:3000/api/attendance/manual', 'POST', attendanceData);
