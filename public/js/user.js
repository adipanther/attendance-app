...
async function checkTodayStatus() {
    try {
        const data = await apiCall('/api/attendance/today/status');
...
async function checkIn() {
    ...
    navigator.geolocation.getCurrentPosition(
        async (position) => {
            try {
                const data = await apiCall('/api/attendance/checkin', 'POST', {
...
async function checkOut() {
    ...
    navigator.geolocation.getCurrentPosition(
        async (position) => {
            try {
                const data = await apiCall('/api/attendance/checkout', 'POST', {
                    latitude: position.coords.latitude,
                    longitude: position.coords.longitude
                });
